...
Then we want to plot the PC coordinates.

To sucessfully generate the plot without a graphic design error, we needed to use Cairo, creating the plot and saving it as a png, using the cairo graphic rendering device.

Then add the plot to the quarto document, via knitr, to embed extrenal images.

```{r}
png("pca_plot.png", type="cairo")
